...
from services.vault import test_b2_connection, archive_to_b2, get_presigned_streaming_url, dispatch_webhook_notification, create_and_upload_storyboard_zip
...
st.sidebar.markdown('<div class="sidebar-title">🌌 GenMedia Control</div>', unsafe_allow_html=True)
...
if has_byok:
    ...
    if tries_used >= 10:
        st.sidebar.error("❌ Free tries exhausted! Enter a HF Token to enable unlimited generations.")

# Backblaze B2 Bucket Credentials
st.sidebar.markdown("---")
st.sidebar.subheader("💾 Backblaze B2 Vault Setup")
b2_id = st.sidebar.text_input("B2 Application Key ID", value=st.session_state["b2_key_id"], type="password")
b2_key = st.sidebar.text_input("B2 Application Key", value=st.session_state["b2_application_key"], type="password")
b2_bucket = st.sidebar.text_input("B2 Bucket Name", value=st.session_state["b2_bucket_name"])
...
if b2_configured:
    ...
    if not (3 <= len(bucket_stripped) <= 50) or not re.match(r"^[a-zA-Z0-9\-]+$", bucket_stripped) or bucket_stripped.startswith("-") or bucket_stripped.endswith("-") or "--" in bucket_stripped:
...
with st.sidebar.expander("System package diagnostics"):
    for pkg in health_report:
        ...
        st.markdown(f"**{pkg['package']}**: :{color}[{pkg['status']}] (v{pkg['version']})")
        
    st.markdown("---")
    st.markdown("**Pip Conflict Scanner**")
    dump = st.text_area("Paste terminal logs to diagnose", placeholder="requires numpy but you have...", key="diagnostic_dump")
...
def verify_rate_limit():
    if not has_byok and st.session_state["tries_used"] >= 10:
        st.error("Rate limit threshold of 10 free tries reached! Enter your Hugging Face API Token (BYOK) in the sidebar to run unlimited generations.")
        return False
    return True
...
with tab1:
    st.markdown('<div class="section-header">🎨 Manga Workspace</div>', unsafe_allow_html=True)
    ...
    with col1:
        st.write("Route image compiles to any FLUX or image model identifier dynamically via the central orchestrator.")
        ...
        style_preset = st.selectbox(
            ...
            options=["Raw Prompt (No Preset)", "Classic Ink Manga", "Retro 90s Anime", "Chibi Slice-of-Life", "Cyberpunk Neon Panel"],
        ...
        if st.button("🚀 Compile Manga Panel", key="compile_manga_btn", disabled=is_disabled, use_container_width=True):
            if verify_rate_limit():
                ...
                with st.spinner("Compiling panel via central orchestrator..."):
                    ok, res_path = compile_manga_panel(token, final_prompt, model_id=manga_model)
                    if ok:
        ...
        if is_disabled:
            st.info("💡 Pro-Tip: Provide your own Hugging Face token in the sidebar to bypass the 10 free tries limit.")
    ...
    with col2:
        ...
        if st.session_state["manga_image"] is not None:
            ...
            st.session_state["manga_filename"] = st.text_input("B2 Target Filename", value=st.session_state["manga_filename"], key="manga_filename_input")
        else:
            st.info("No manga panel compiled yet. Enter a prompt and compile above to generate!")

# ==================== TAB 2: LIGHT NOVEL FACTORY ====================
with tab2:
    st.markdown('<div class="section-header">📖 Light Novel Factory</div>', unsafe_allow_html=True)
    ...
    with ln_tabs[0]:
        ...
        with col1:
            st.write("Orchestrate open-source LLM writes and English cross-translations using dynamic model endpoints.")
            ...
            ln_title = st.text_input("Chapter/Story Title", value="運命のコンパイル (Destiny Compile)", key="ln_title_input")
            ...
            ln_genre = st.selectbox("Genre Style", ["Isekai (Otherworld Fantasy)", "Cyberpunk Slice of Life", "School Romance", "Shonen Action", "Psychological Horror"], key="ln_genre_select")
            ln_tone = st.select_slider("Writing Tone", options=["Humorous & Light", "Expressive & Standard", "Dark & Moody", "Epic & Grandiose"], key="ln_tone_slider")
            
            is_disabled_ln = (not has_byok and tries_used >= 10)
            
            if st.button("✍️ Write Japanese Scene", key="write_ln_btn", disabled=is_disabled_ln, use_container_width=True):
                if verify_rate_limit():
                    prompt_instructions = (
                    ...
                    with st.spinner("Generating Japanese novel & English translation via central orchestrator..."):
                        ok, jp_res, en_res = write_japanese_novel_scene(token, prompt_instructions, model_id=novel_model)
                        if ok:
                            st.session_state["light_novel_jp"] = jp_res
                            st.session_state["light_novel_en"] = en_res
                            secure_increment_tries(2)
                            st.success("Japanese scene generated and chained English translation completed!")
                        else:
        ...
        with col2:
            ...
            st.markdown("<h4 style='margin-top:0;'>🇯🇵 Japanese Original Original Text</h4>", unsafe_allow_html=True)
            ...
            st.session_state["light_novel_jp_filename"] = st.text_input("B2 Japanese Text Filename", value=st.session_state["light_novel_jp_filename"], key="ln_jp_filename_input")
            st.markdown('</div>', unsafe_allow_html=True)
            
            # Column EN
            st.markdown('<div class="glass-card-neon-pink">', unsafe_allow_html=True)
            st.markdown("<h4 style='margin-top:0;'>🇺🇸 English Translation Frame</h4>", unsafe_allow_html=True)
            ...
            st.session_state["light_novel_en_filename"] = st.text_input("B2 English Text Filename", value=st.session_state["light_novel_en_filename"], key="ln_en_filename_input")
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    with ln_tabs[1]:
        ...
        src_text = st.text_area("Source Text to Translate", height=150, placeholder="Enter text in English or Japanese...", key="direct_src_input")
        tr_direction = st.radio("Translation Direction", ["Japanese ➔ English", "English ➔ Japanese"], key="direct_direction_radio")
        
        if st.button("Translate", key="direct_translate_btn", disabled=is_disabled_ln, use_container_width=True):
            if src_text.strip():
                if verify_rate_limit():
                    token = get_active_token()
                    direction_instruction = "Japanese into natural English" if tr_direction == "Japanese ➔ English" else "English into natural, authentic Japanese light novel style prose"
                    prompt_trans = (
                    ...
                    with st.spinner("Translating text..."):
                        ok, res_text = translate_novel_text(token, prompt_trans, model_id=translation_model)
                        if ok:
...
with tab3:
    st.markdown('<div class="section-header">🎙️ Whisper Subtitle Studio</div>', unsafe_allow_html=True)
    ...
    with col1:
        st.write("Run speech-to-text audio transcriptions using Whisper or other audio recognition models dynamically.")
        ...
        audio_file = st.file_uploader("Upload Audio Track", type=["mp3", "wav", "m4a", "ogg"], key="whisper_audio_upload")
        ...
        use_demo_audio = st.checkbox("Use Simulated Audio Track (if no file is uploaded)", key="whisper_demo_checkbox")
        
        is_disabled_whisper = (not has_byok and tries_used >= 10)
        
        if st.button("🎙️ Process Speech-To-Text", key="process_whisper_btn", disabled=is_disabled_whisper, use_container_width=True):
            if verify_rate_limit():
                token = get_active_token()
                
                if (audio_data is not None) or use_demo_audio:
                    with st.spinner("Processing audio transcription..."):
                        ok, text_out, srt_out = transcribe_audio(token, audio_data, model_id=whisper_model)
                        if ok:
                    ...
                    st.warning("Please upload an audio file or record some audio first.")
    ...
    with col2:
        ...
        if st.session_state["whisper_transcript"]:
            ...
            st.session_state["whisper_filename"] = st.text_input("B2 Subtitles Filename", value=st.session_state["whisper_filename"], key="whisper_filename_input")
        else:
            st.info("No transcription processed yet. Upload or record audio and hit Process to transcribe.")

# ==================== TAB 4: AGENT CONTINUITY STUDIO ====================
with tab4:
    st.markdown('<div class="section-header">🤖 Agent Continuity Studio</div>', unsafe_allow_html=True)
    ...
    st.write("Orchestrate an advanced Genblaze Agent Loop for multi-panel continuity generation. Evaluates consistency against a template and dynamically refines parameters on failure.")
    ...
    with col_options:
        ...
        for i in range(5):
            with st.expander(f"Panel {i+1} Configuration", expanded=(i==0)):
                img_p = st.text_input(f"Panel {i+1} Image Prompt", value=default_img_prompts[i], key=f"agent_img_p_{i}")
                aud_p = st.text_input(f"Panel {i+1} Audio Prompt", value=default_aud_prompts[i], key=f"agent_aud_p_{i}")
                user_img_prompts.append(img_p)
                user_aud_prompts.append(aud_p)
                
        if st.button("⚡ Run Continuity Agent Loop", key="run_agent_loop_btn", disabled=is_disabled_agent, use_container_width=True):
            if verify_rate_limit():
                token = get_active_token()
                with st.spinner("Executing Genblaze Agent Loop (Generates concurrent steps & evaluates visual consistency)..."):
                    res = run_agent_loop(
                        token=token,
                    ...
                    if res.get("success"):
                        ...
                        st.success(f"Agent Loop completed after {run_iters} iteration(s)!")
                    else:
        ...
        if "agent_storyboard_result" in st.session_state:
            ...
            with st.expander("Show Agent Loop Refinement Logs", expanded=True):
                for iter_data in res["iterations"]:
                    ...
                    st.markdown(f"**Iteration {iter_data['iteration']}**: Score: `{iter_data['score']:.2f}` | Status: {p_status}")
                ...
                with st.container():
                    ...
                    with col_p_img:
                        if p["image_path"] and os.path.exists(p["image_path"]):
                            st.image(p["image_path"], use_container_width=True, caption=p["image_prompt"])
                        else:
            ...
            if not b2_configured:
                ...
                with col_b2_arch:
                    if st.button("📤 Upload Media Array & Manifest to B2 Concurrently", key="vault_agent_storyboard_btn", use_container_width=True):
                        with st.spinner("Pushing verified media array to B2 concurrently..."):
                            ...
                            if ok:
                                st.success("🎉 Storyboard assets and run manifest archived successfully!")
                                st.session_state["last_upload_reports"] = reports
                            else:
                                st.error(f"Concurrent upload failed: {msg}")

                with col_b2_zip:
                    if st.button("📦 Package & Upload Storyboard (.zip) Bundle", key="zip_agent_storyboard_btn", use_container_width=True):
                        with st.spinner("Compiling panels, audio, subtitles & C2PA manifests into .zip bundle..."):
                            # Construct bundle items
                            bundle_items = {}
                            for p in res.get("panels", []):
                                idx = p["panel_index"]
                                if p["image_path"] and os.path.exists(p["image_path"]):
                                    bundle_items[f"panel_{idx}"] = {"name": f"manga_panel_{idx+1}.png", "data": Image.open(p["image_path"]), "type": "image"}
                                if p["audio_path"] and os.path.exists(p["audio_path"]):
                                    with open(p["audio_path"], "rb") as f:
                                        aud_bytes = f.read()
                                    bundle_items[f"audio_{idx}"] = {"name": f"audio_track_{idx+1}.wav", "data": aud_bytes, "type": "audio"}

                            if st.session_state.get("whisper_srt"):
                                bundle_items["subtitles"] = {"name": "subtitles.srt", "data": st.session_state["whisper_srt"], "type": "text"}
                            if st.session_state.get("light_novel_en"):
                                bundle_items["novel_en"] = {"name": "light_novel_en.txt", "data": st.session_state["light_novel_en"], "type": "text"}

                            ok_z, msg_z, p_url_z, z_bytes, report_z = create_and_upload_storyboard_zip(
                                b2_id=b2_id,
                                b2_key=b2_key,
                                b2_bucket=b2_bucket,
                            ...
                            if ok_z:
                                ...
                                st.success("🎉 Storyboard .zip bundle created & uploaded to B2 Vault!")
                                if p_url_z:
                                    st.markdown(f"**Presigned Download URL:** [B2 Direct Zip Link]({p_url_z})")
                                st.download_button(
                                    label="📥 Direct Download (.zip) Bundle",
                                    data=z_bytes,
                                    file_name=report_z.get("filename", "storyboard_bundle.zip"),
            ...
            st.info("No Storyboard generation executed yet. Customize panels and run the Agent Loop to render!")

# ==================== TAB 5: BACKBLAZE B2 VAULT ARCHIVE ====================
with tab5:
    st.markdown('<div class="section-header">🗄️ Backblaze B2 Vault Archive & Spatial Time Travel</div>', unsafe_allow_html=True)
    
    if not b2_configured:
        st.warning("⚠️ Backblaze B2 Credentials not configured in the sidebar. Please configure Key ID, Key, and Bucket Name to connect.")
    else:
        ...
        if not (3 <= len(bucket_stripped) <= 50) or not re.match(r"^[a-zA-Z0-9\-]+$", bucket_stripped) or bucket_stripped.startswith("-") or bucket_stripped.endswith("-") or "--" in bucket_stripped:
            ...
            st.write("Query historical snapshots and roll back session states instantly.")
            
            enable_time_travel = st.checkbox("Enable B2 Spatial Time Travel Control", value=False, key="enable_time_travel")
            
            if enable_time_travel:
                with st.spinner("Querying historical versions from B2 vault..."):
                    ok_v, msg_v, versions = list_historical_versions(b2_id, b2_key, b2_bucket)
                    
                if not ok_v:
                    st.error(f"Failed to fetch B2 history: {msg_v}")
                elif not versions:
                    st.info("No historical file versions found in your B2 bucket. Upload assets first!")
                else:
                    st.success(f"Discovered {len(versions)} historical file version(s).")
                    ...
                    if st.button("🔄 Roll Back Session State to this Version", key="restore_time_travel_btn", use_container_width=True):
                        # Verify integrity before execution to preserve cryptographic safety
                        sandbox_check = sandbox.verify_integrity(st.session_state["tries_used"], st.session_state.get("tries_signature", ""))
                        if not sandbox_check:
                            st.error("Security Sandbox Integrity Violation. Operation blocked.")
                            st.stop()
                            
                        with st.spinner(f"Retrieving and restoring '{selected_ver['file_name']}' from B2..."):
                            ok_dl, file_bytes = download_historical_file(b2_id, b2_key, selected_ver["file_id"])
                            if ok_dl:
                                fname = selected_ver["file_name"]
                                # Roll back memory states
                                if fname.endswith(".png"):
                                    st.session_state["manga_image"] = Image.open(io.BytesIO(file_bytes))
                                    st.session_state["manga_filename"] = fname
                                    st.success("🎨 Manga panel state restored successfully!")
                                elif fname.endswith("light_novel_jp.txt") or fname.endswith("_jp.txt"):
                                    st.session_state["light_novel_jp"] = file_bytes.decode("utf-8")
                                    st.session_state["light_novel_jp_filename"] = fname
                                    st.success("🇯🇵 Light Novel Japanese state restored successfully!")
                                elif fname.endswith("light_novel_en.txt") or fname.endswith("_en.txt"):
                                    st.session_state["light_novel_en"] = file_bytes.decode("utf-8")
                                    st.session_state["light_novel_en_filename"] = fname
                                    st.success("🇺🇸 Light Novel English state restored successfully!")
                                elif fname.endswith(".srt") or fname.endswith("subtitles.srt"):
                                    st.session_state["whisper_srt"] = file_bytes.decode("utf-8")
                                    st.session_state["whisper_filename"] = fname
                                    st.success("📝 Subtitles SRT state restored successfully!")
                                else:
                                    # Fallback text restore
                                    try:
                                        text_content = file_bytes.decode("utf-8")
                                        st.session_state["whisper_transcript"] = text_content
                                        st.success(f"Text content of '{fname}' restored into transcription memory!")
                                    except Exception:
                                        st.warning("Downloaded file is binary and couldn't be routed to text states.")
                            else:
                                st.error(f"Download failed: {file_bytes.decode('utf-8')}")
            st.markdown('</div>', unsafe_allow_html=True)
            st.markdown("---")
            
            st.success("🔌 Backblaze B2 configuration detected. Ready to archive assets.")
            ...
            with col1:
                ...
                c1.write(f"🖼️ **Manga Panel Image** (`{st.session_state['manga_filename']}`)")
                if manga_ready:
                    c2.markdown('<div class="status-badge ready">Ready</div>', unsafe_allow_html=True)
                    manga_select = st.checkbox("Include Manga Panel in Archive", value=True, key="manga_archive_checkbox")
                    if manga_select:
                        archive_items["manga"] = {
                    ...
                    c2.markdown('<div class="status-badge empty">Empty</div>', unsafe_allow_html=True)
                    
                # Asset 2: LN Japanese Text
                ln_jp_ready = bool(st.session_state["light_novel_jp"].strip())
                c1, c2 = st.columns([3, 1])
                c1.write(f"🇯🇵 **Light Novel Japanese Text** (`{st.session_state['light_novel_jp_filename']}`)")
                if ln_jp_ready:
                    c2.markdown('<div class="status-badge ready">Ready</div>', unsafe_allow_html=True)
                    ln_jp_select = st.checkbox("Include LN Japanese text in Archive", value=True, key="ln_jp_archive_checkbox")
                    if ln_jp_select:
                        archive_items["ln_jp"] = {
                ...
                c1, c2 = st.columns([3, 1])
                c1.write(f"🇺🇸 **Light Novel English Text** (`{st.session_state['light_novel_en_filename']}`)")
                if ln_en_ready:
                    c2.markdown('<div class="status-badge ready">Ready</div>', unsafe_allow_html=True)
                    ln_en_select = st.checkbox("Include LN English text in Archive", value=True, key="ln_en_archive_checkbox")
                    if ln_en_select:
                        archive_items["ln_en"] = {
                ...
                c1, c2 = st.columns([3, 1])
                c1.write(f"📝 **Whisper Subtitle Manifest** (`{st.session_state['whisper_filename']}`)")
                if srt_ready:
                    c2.markdown('<div class="status-badge ready">Ready</div>', unsafe_allow_html=True)
                    srt_select = st.checkbox("Include Subtitle SRT in Archive", value=True, key="whisper_archive_checkbox")
                    if srt_select:
                ...
                # Action Button
                if archive_items:
                    if st.button("📤 Archive Selected Assets to B2 Vault", key="archive_to_b2_btn", use_container_width=True):
                        with st.spinner("Pushing assets to Backblaze B2 Vault via modular vault service..."):
                            ok, msg, reports = archive_to_b2(b2_id, b2_key, b2_bucket, archive_items)
                            if ok:
                                st.session_state["last_upload_reports"] = reports
                                st.success("🎉 All selected assets successfully archived to your Backblaze B2 Vault!")
                            else:
                                st.error(f"Archiving failed: {msg}")
                else:
                    st.info("No compiled assets are selected or available for archiving. Generate assets in tabs above first!")
            ...
            vault_lineage_data = st.session_state.get("agent_storyboard_result") or {
                "master_prompt": st.session_state.get("agent_master_prompt", "Backblaze B2 Vault Media Archive"),
                "iterations": [{"iteration": 1, "score": 0.88, "passed": True}],
                "panels": [
                    {
                        "panel_index": 0,
                        "image_path": st.session_state.get("manga_filename") if st.session_state.get("manga_image") else "manga_panel_0.png",
            ...
            st.write("Generate temporary, authenticated public download URLs powered by `b2sdk` for direct high-speed CDN streaming.")

            col_stream_opt, col_stream_player = st.columns([1, 1])

            with col_stream_opt:
                reports = st.session_state.get("last_upload_reports", [])
                file_options = [r["filename"] for r in reports] if reports else [
                    "manga_panel_0.png", "audio_track_0.wav", "storyboard_manifest.txt", "manga_panel.png", "subtitles.srt"
                ]
                ...
                custom_filename = st.text_input("Or Enter Custom B2 Filename", value="", placeholder="e.g. audio_track_0.wav", key="custom_stream_filename")
                target_filename = custom_filename.strip() if custom_filename.strip() else selected_stream_file
                
                duration_hours = st.slider("Presigned URL Validity (Hours)", min_value=1, max_value=24, value=1, key="presigned_duration_slider")
                valid_seconds = duration_hours * 3600

                gen_presigned_btn = st.button("⚡ Generate Direct B2 Presigned Streaming URL", key="gen_presigned_btn", use_container_width=True)

            with col_stream_player:
                if gen_presigned_btn or ("last_presigned_url" in st.session_state and st.session_state.get("last_presigned_filename") == target_filename):
                    with st.spinner("Generating authenticated presigned streaming URL via b2sdk..."):
                    ...
                    if ok_p:
                        ...
                        if fn_lower.endswith(".wav") or fn_lower.endswith(".mp3") or fn_lower.endswith(".ogg") or "audio" in fn_lower:
                        ...
                        elif fn_lower.endswith(".png") or fn_lower.endswith(".jpg") or fn_lower.endswith(".jpeg") or fn_lower.endswith(".webp") or "panel" in fn_lower or "manga" in fn_lower:
                            ...
                            st.markdown(f"[📥 Click to Stream/Download {target_filename} directly from Backblaze B2]({presigned_res})")
                    else:
                        st.error(f"Failed to generate presigned streaming URL: {presigned_res}")
                else:
                    st.info("Select an asset and click 'Generate Direct B2 Presigned Streaming URL' to stream media directly from Backblaze B2 CDN!")
            ...
            st.write("Bundles all generated manga panels, audio tracks, translated light novel texts, SRT subtitle manifests, and canonical C2PA provenance JSON manifests into a single compressed `.zip` file.")

            col_zip_opt, col_zip_act = st.columns([1.5, 1])

            with col_zip_opt:
                zip_filename_input = st.text_input("Custom Bundle Zip Filename", value=f"storyboard_bundle_{int(time.time())}.zip", key="zip_bundle_filename_input")

            with col_zip_act:
                if st.button("📦 Package, Upload & Generate Zip Link", key="tab5_create_zip_btn", use_container_width=True):
                    with st.spinner("Zipping all active session assets & uploading to B2..."):
                        # Gather all available assets in session
                        b_items = {}
                        if st.session_state.get("manga_image"):
                            b_items["manga"] = {"name": st.session_state["manga_filename"], "data": st.session_state["manga_image"], "type": "image"}
                        if st.session_state.get("light_novel_jp"):
                            b_items["ln_jp"] = {"name": st.session_state["light_novel_jp_filename"], "data": st.session_state["light_novel_jp"], "type": "text"}
                        if st.session_state.get("light_novel_en"):
                            b_items["ln_en"] = {"name": st.session_state["light_novel_en_filename"], "data": st.session_state["light_novel_en"], "type": "text"}
                        if st.session_state.get("whisper_srt"):
                            b_items["srt"] = {"name": st.session_state["whisper_filename"], "data": st.session_state["whisper_srt"], "type": "text"}
                        ...
                        if "agent_storyboard_result" in st.session_state:
                            ...
                            for p in res_s.get("panels", []):
                                idx_s = p["panel_index"]
                                if p["image_path"] and os.path.exists(p["image_path"]):
                                    b_items[f"sb_panel_{idx_s}"] = {"name": f"manga_panel_{idx_s+1}.png", "data": Image.open(p["image_path"]), "type": "image"}
                                if p["audio_path"] and os.path.exists(p["audio_path"]):
                                    with open(p["audio_path"], "rb") as f:
                                        aud_b = f.read()
                                    b_items[f"sb_audio_{idx_s}"] = {"name": f"audio_track_{idx_s+1}.wav", "data": aud_b, "type": "audio"}

                        if not b_items:
                            st.warning("No session assets available to zip. Compile assets in tabs above first!")
                        else:
                            ok_z, msg_z, p_url_z, z_bytes, report_z = create_and_upload_storyboard_zip(
                                b2_id=b2_id,
                                b2_key=b2_key,
                            ...
                            if ok_z:
                                ...
                                st.session_state["tab5_zip_name"] = zip_filename_input.strip()
                                st.success("🎉 Complete Storyboard Bundle (.zip) created & archived to B2 Vault!")
                            else:
                                st.error(f"Failed to create zip bundle: {msg_z}")
            ...
            if "tab5_zip_bytes" in st.session_state:
                ...
                with col_z1:
                    ...
                    st.markdown(f"[📥 Direct B2 CDN Zip Download Link]({st.session_state['tab5_zip_url']})")
                with col_z2:
                    st.download_button(
                        label="📥 Download (.zip) Bundle Directly to Device",
                        data=st.session_state["tab5_zip_bytes"],
                        file_name=st.session_state.get("tab5_zip_name", "storyboard_bundle.zip"),
                        mime="application/zip",
                        key="dl_tab5_zip_btn",
            ...
            st.write("Publish verified assets, B2 presigned streaming URLs, and C2PA provenance metadata to Discord, Zapier, Make, or custom REST webhooks.")
            ...
            with col_wh_input:
                ...
                publish_asset_name = st.text_input(
                    ...
                    value=st.session_state.get("last_presigned_filename", "storyboard_panel_0.png"),
                    key="publish_asset_name_input"
                )

            with col_wh_action:
                st.write("**Payload Contents:**")
                st.caption("• C2PA Provenance Manifest\n• B2 Presigned Streaming URL\n• Cryptographic SHA-256 Signature")

                if st.button("🚀 Dispatch Webhook Payload", key="dispatch_webhook_btn", use_container_width=True):
                    if not target_webhook_url.strip():
                        st.warning("Please enter a valid Webhook URL endpoint first.")
                    else:
                        with st.spinner("Dispatching publication payload to external webhook..."):
                            # Get presigned URL or generate one for active file
                            p_stream_url = st.session_state.get("last_presigned_url", "")
                            if not p_stream_url and b2_configured:
                                _, p_stream_url = get_presigned_streaming_url(b2_id, b2_key, b2_bucket, publish_asset_name)

                            # Gather C2PA metadata
                            prov_meta = {
                                "c2pa_spec": "C2PA-v1.3-GenMedia",
                                "prompt": st.session_state.get("agent_master_prompt", "Cyberpunk detective scene"),
                                "seed": 42981,
                            ...
                            webhook_payload = {
                                ...
                                "presigned_url": p_stream_url or "https://f000.backblazeb2.com/file/demo/asset.png",
                            ...
                            ok_wh, msg_wh, res_wh = dispatch_webhook_notification(target_webhook_url, webhook_payload)

                            if ok_wh:
                                st.success(f"🎉 {msg_wh}")
                                with st.expander("Show Webhook Response Payload", expanded=True):
                                    st.json(res_wh)
                            else:
                                st.error(f"❌ {msg_wh}")
...
with tab6:
    st.markdown('<div class="section-header">🔒 Secure Code Inspector</div>', unsafe_allow_html=True)
    st.write("Displays `app.py` in real-time, dynamically sanitizing all private auth keys and access credentials.")
